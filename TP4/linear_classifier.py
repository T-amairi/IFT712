...
class LinearClassifier(object):
    ...
    def predict(self, X):
        ...
        #############################################################################
        if self.bias & (np.shape(X)[1] < np.shape(self.W)[0]):
            X = augment(X)

        out = np.dot(X, self.W)
        out = np.exp(out)
        out /= np.sum(out, axis=1)[np.newaxis].T
        class_label = np.argmax(out, axis=1)
        #############################################################################
        #                          END OF YOUR CODE                                 #
        #############################################################################
        return class_label
    ...
    def global_accuracy_and_cross_entropy_loss(self, X, y, reg=0.0):
        ...
        for n in range(X.shape[0]):
            sample_loss, dw = self.cross_entropy_loss(X[n], y[n], reg)
            loss += sample_loss
        ...
        class_label = self.predict(X)
        accu = (class_label == y).mean()
        #############################################################################
        #                          END OF YOUR CODE                                 #
        #############################################################################
        return accu, loss
    ...
    def cross_entropy_loss(self, x, y, reg=0.0):
        ...
        out = np.exp(out)
        out /= np.sum(out)
        onehotVector = np.zeros((y.size,self.num_classes))
        onehotVector[np.arange(y.size), y] = 1
        loss = -np.sum(onehotVector * np.log(out)) + reg * np.sum(self.W ** 2)
        dW = np.dot(x[np.newaxis].T,(out - onehotVector)) + reg * self.W
        #############################################################################
        #                          END OF YOUR CODE                                 #
        #############################################################################
        return loss, dW
