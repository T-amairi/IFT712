...
class TwoLayerClassifier(object):
    ...
    def global_accuracy_and_cross_entropy_loss(self, x, y, l2_r=-1.0):
        ...
        for x_s, y_s in zip(x, y):
            scores = self.net.forward(x_s)
            loss_s, _ = self.net.cross_entropy_loss(scores, y_s)
            loss += loss_s
        loss /= len(x)
...
class TwoLayerNet(object):
    ...
    def cross_entropy_loss(self, scores, y):
        ...
        s = np.exp(scores)
        s/=s.sum()
        t = onehot(y,self.num_classes)[0]

        loss = -np.sum( t * np.log(s))
        for p in self.parameters:
            loss +=  np.sum(self.l2_reg * (np.linalg.norm(p))**2)
        dloss_dscores = s - t
        #############################################################################
        #                          END OF YOUR CODE                                 #
        #############################################################################
