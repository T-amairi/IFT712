...
class MAPnoyau:
    ...
    def entrainement(self, x_train, t_train):
        ...
        if(self.noyau == "lineaire"):
            ...
            K = (x_train @ x_train.T + self.c) ** self.M
        elif(self.noyau == "sigmoidal"):
            K = np.tanh(self.b * x_train @ x_train.T + self.d)
        else:
            K = np.array([np.exp(- (np.linalg.norm(x_train - x_train[i,], axis=1) ** 2) / (2 * self.sigma_square)) 
                for i in range(N)])

        self.a = np.linalg.solve(K + self.lamb * np.eye(N), t_train)
    ...
    def prediction(self, x):
        ...
        if(self.noyau == "lineaire"):
            ...
            kx = (x @ self.x_train.T + self.c) ** self.M
        elif(self.noyau == "sigmoidal"):
            kx = np.tanh(self.b * x @ self.x_train.T + self.d)
        else:
            kx = np.exp(-(np.linalg.norm(self.x_train - x,axis=1) ** 2) / (2 * self.sigma_square))
    ...
    def validation_croisee(self, x_tab, t_tab):
        ...
        """
        def getError(Xtest, Ytest):
            pred = np.array([self.prediction(Xtest[i,]) for i in range(Xtest.shape[0])])
            return np.mean(self.erreur(Ytest, pred))

        def getCVErr(data, idxSplit):
            errList = list()
            idxTrain = np.ones(data.shape[0],dtype=bool)

            for idxTest in idxSplit:
                idxTrain[idxTest] = 0
                Xtrain, YTrain = data[:,1:][idxTrain], data[:,0][idxTrain]
                Xtest, YTest = data[:,1:][idxTest], data[:,0][idxTest]
                self.entrainement(Xtrain, YTrain)
                errList.append(getError(Xtest, YTest))
                idxTrain[:] = True

            return np.mean(errList)

        data = np.c_[t_tab, x_tab]
        np.random.shuffle(data)

        k = 10
        idxSplit = np.array_split(np.arange(0,data.shape[0]), k)
        
        err = float("inf")
        hyperPara = [self.lamb, self.sigma_square, self.M, self.c, self.b, self.d]

        lambVal = np.linspace(0.000000001, 2, 10)
        sigmaSquareVal = np.linspace(0.000000001, 2, 10)
        bVal = np.linspace(0.00001, 0.01, 5)
        dVal = np.linspace(0.00001, 0.01, 5)
        cVal = np.arange(0,6,1)
        MVal = np.arange(2,7,1)

        for lamb in lambVal:
            self.lamb = lamb

            if(self.noyau == "lineaire"):
                CVErr = getCVErr(data, idxSplit)
                if err > CVErr:
                    err = CVErr
                    hyperPara[0] = lamb
                ...
                for sig in sigmaSquareVal:
                    self.sigma_square = sig
                    CVErr = getCVErr(data, idxSplit)
                    if err > CVErr:
                        err = CVErr
                        hyperPara[0] = lamb
        ...
        self.lamb = hyperPara[0]
        self.sigma_square = hyperPara[1]
        self.M = hyperPara[2]
        self.c = hyperPara[3]
        self.b = hyperPara[4]
        self.d = hyperPara[5]
